...
public class MapTemplate {
    ...
    public static Object apply(String string, Map<String, Object> variableMap) {
        if (!string.contains(START_VARIABLE) || !string.contains(END_VARIABLE)) {
            return string;
        }
        ...
        while (i < string.length()) {
            int start = string.indexOf(START_VARIABLE, i);
            if (start == -1) {
            ...
            if (start > 0 && string.startsWith(ESCAPE_CHAR, start - ESCAPE_CHAR.length())) {
                int escapeCount = 0;
                int j = start - 1;
                while (j >= 0 && string.startsWith(ESCAPE_CHAR, j)) {
                    escapeCount++;
                    j--;
                }
                if (escapeCount % 2 != 0) {
            ...
            int end = string.indexOf(END_VARIABLE, start + START_VARIABLE.length());
            boolean matched = false;
            while (end != -1) {
                String key = string.substring(start + START_VARIABLE.length(), end);
                ...
                end = string.indexOf(END_VARIABLE, end + END_VARIABLE.length());
            }
